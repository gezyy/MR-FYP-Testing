...
public class GBController : MonoBehaviour
{
    ...
    public GameObject G1; // 存储R1的引用
    public GameObject G2; // 存储R2的引用
    public GameObject G3; // 存储R3的引用
    public GameObject G4; // 存储R3的引用
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Bullet"))
        {
            ...
            if (gameObject == G1)
            {
                ActivateSubObject("G1");
            }
            else if (gameObject == G2)
            {
                ActivateSubObject("G2");
            }
            else if (gameObject == G3)
            {
                ActivateSubObject("G3");
            }
            else if (gameObject == G4)
            {
                ActivateSubObject("G4");
            }
            // 禁用当前小球物体
